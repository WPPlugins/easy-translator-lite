...
if (!class_exists("EasyTranslator")) {
  ...
  class EasyTranslator extends EzBasePlugin {
    ...
    var $domain, $eztPlgSlug, $eztPlgName, $eztMoName, $eztPlgDir, $eztAllDomains,
            $eztDomain, $eztAllMoFiles, $eztMoFile, $eztLocale, $eztPoName, $eztDelegate;
    ...
    function handleSubmits() {
      ...
      if (isset($_POST['ezt-loadmo']) || isset($_POST['ezt-createpo'])) {
        ...
        else if (!empty($_POST['ezt-mofile'])) {
    ...
    function printToolPage() {
      ...
      echo "<input type='hidden' name='ezt-name' id='eztPlgName' value='{$this->eztPlgName}' />";
      $loadPlugin = "&nbsp; <input class='button-primary' type='submit' style='width:10%' name='ezt-load' value='Load it' title='Looks for all the MO files of the selected plugin' onmouseover = 'Tip(\"Looks for all language (MO) files of the selected plugin\",WIDTH, 300)' onmouseout='UnTip()'/> <br /><br />\n";
      echo $loadPlugin;
...
<input type="text" style="width: 40%" name="ezt-domain" id="domain" value="$domain"  title="Enter the text-domain used by this plugin -- usually plugin-name" />&nbsp; <input class='button-primary' type="button" style="width:10%" title="Click for more info" onclick="Tip('$info',WIDTH, 300)" onmouseover = "Tip('$info',WIDTH, 300)" onmouseout="UnTip()" value="Info" /><br />
EOF;
      echo $textDomain;
      ...
      $loadmo = "&nbsp; <input class='button-primary' type='submit' style='width:10%' name='ezt-loadmo' value='Load MO' title='$tip' onmouseover = 'Tip(\"$tip\",WIDTH, 300)' onmouseout='UnTip()' /><br /><br />\n";
      echo $loadmo;
    ...
    function printFooter() {
      ...
      <div class='updated' id='rating'>
        <p>Thank you for using <i><b>Easy Translator</b></i>! You will find it feature-rich and robust. <br />
          If you are satisfied with how well it works, why not <a href='http://wordpress.org/extend/plugins/easy-translator-lite/' onclick="popupwindow('http://wordpress.org/extend/plugins/easy-translator-lite/', 'Rate it', 1024, 768);
              return false;">rate it</a>
          and <a href='http://wordpress.org/extend/plugins/easy-translator-lite/' onclick="popupwindow('http://wordpress.org/extend/plugins/easy-translator-lite/', 'Rate it', 1024, 768);
              return false;">recommend it</a> to others? :-)
        </p></div>
      <?php
      $ez->renderSupport();
      $ez->renderTailText();
          ...
          <tr style="vertical-align:top">
            <td style="width:40%">
              ...
              <ul style='padding-left:10px;list-style-type:circle; list-style-position:inside;'>
                ...
                <li>
                  Plugin Translator: Use <a href="tools.php?page=easy-translator-lite/easy-translator-lite.php">Tools &rarr; Easy Translator</a>
                </li>
              </ul>
            </td>

      <?php $ez->renderHeadText(); ?>
